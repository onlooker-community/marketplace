# Counsel Improvement Brief — {{date}}

**Lookback period:** {{lookback_days}} days (since {{cutoff_date}})

---

## This Week's Friction by Layer

### Belief / Memory layer (Archivist)
{{archivist_findings}}
- **Recommended action:** {{archivist_action}}

### Planning layer (Oracle + Scribe)
{{planning_findings}}
- **Recommended action:** {{planning_action}}

### Safety layer (Sentinel + Warden)
{{safety_findings}}
- **Recommended action:** {{safety_action}}

### Reflection layer (Tribunal + Echo)
{{reflection_findings}}
- **Recommended action:** {{reflection_action}}

### Observability layer (Onlooker)
{{observability_findings}}
- **Recommended action:** {{observability_action}}

---

## Top 3 Friction Points
1. {{friction_1}}
2. {{friction_2}}
3. {{friction_3}}

## Rubric Criteria Failing Most Often
{{rubric_failures}}

## Echo Regression Trends
{{echo_regressions}}

## Summary
{{summary}}
